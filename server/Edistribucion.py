...
class Edistribucion:
    ...
    def __get_url(
        self, url, get=None, post=None, json=None, cookies=None, headers=None
    ):
        __headers = {
        ...
        if "json" in r.headers["Content-Type"]:
            ...
            if jr["actions"][0]["state"] != "SUCCESS":
                if not recursive:
                    ...
                    raise EdisError(
                        "Error processing command: {}".format(
                            jr["actions"][0]["error"][0]["message"]
                        )
                    )
    ...
    def __check_tokens(self):
        ...
        return (
            self.__token != "undefined"
            and self.__access_date + timedelta(minutes=10) > datetime.now()
        )
    ...
    def __force_login(self, recursive=False):
        ...
        r = self.__get_url(
            "https://zonaprivada.edistribucion.com/areaprivada/s/login?ec=302&startURL=%2Fareaprivada%2Fs%2F"
        )
        ...
        action = EdistribucionMessageAction(
            91, "LightningLoginFormController/ACTION$login", "WP_LoginForm", params
        )
        ...
        logging.info(
            "Received name: %s (%s)",
            r["Name"],
            r["visibility"]["Visible_Account__r"]["Identity_number__c"],
        )
    ...
    def get_active_cups(self):
        return list(filter(lambda x: x["Active"] is True, self.get_list_cups()))[0]
